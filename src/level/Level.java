package com.unk.PoC.level;

import java.util.*;
import com.unk.PoC.*;
import com.unk.PoC.entities.*;
import com.unk.PoC.gui.Bitmap;
import com.unk.PoC.level.block.*;
import com.unk.PoC.menu.GotLootMenu;
...
public abstract class Level {
	...
	public static Level loadLevel(Game game, String name) {
		...
		try {
			String imgPath = "level/" + name + ".png";
			Bitmap img = Art.recoverBitmap(imgPath);
		...
		//	img.getRGB(0, 0, w, h, pixels, 0, w);
			...
			level.init(game, name, img.width, img.height, img.pixels);
	...
	private static Level byName(String name) {
		try {
			...
			return (Level) Class.forName("com.unk.PoC.level." + name + "Level").newInstance();
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
